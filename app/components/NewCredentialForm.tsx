...
import { getUser, loadTokens, saveToken } from '~/routes/_auth/user.server'
...
export async function handleCredentialLoader({ request }: LoaderFunctionArgs) {
  ...
  if (parsedUrl.searchParams.get('code')) {
    ...
    return redirect(
      `/quickstart/alerts?token=${encodeURIComponent(scopedToken.token)}&scope=${encodeURIComponent(scope)}`
    )
  }
