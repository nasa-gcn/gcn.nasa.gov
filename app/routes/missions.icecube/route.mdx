...
where $$N_\text{signal}$$ and $$N_\text{background}$$ are the expected number of signal and background events (both atmospheric neutrinos and muons) with reconstructed event energy $$E$$ above energy the selected energy proxy $$E_{\text{event}}$$ at declination $$\delta$$, derived from simulations.  
...
- `record_number` – `1` for the first preliminary reconstruction, `2` for the revised reconstruction.  
...
- `systematic_included` – `True` if systematics are included in the positional uncertainty, otherwise `False`. For `"record_number"`=`1` (preliminary reconstruction), this is set to `False`. For `"record_number"`=`2` (updated reconstruction associated to the GCN circular), this is set to `True`. In fact, starting from September 2024, the reconstruction algorithms used for realtime alerts have been improved [^8]. Systematic errors affecting the reconstructions, such as those from the ice model or detector geometry, have been thoroughly investigated to ensure proper statistical coverage (see details in [^9]).
- `healpix_url` – URL to the multi-order probability map of the event; `null` for preliminary reconstruction. For `"record_number"`=`1` (preliminary reconstruction), `healpix_url` is expected to be `null`. For `"record_number"`=`2` (updated reconstruction associated to the GCN circular), it contains a link from which the file can be downloaded. A description of the multi-order probability map of the event is provided in the section "Multi-order Probability Maps (MOC/HEALPix)" below. The probability maps have correct coverage including both statistical and systematic uncertainties.
