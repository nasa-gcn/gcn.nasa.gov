...
where $N_{\mathrm{signal}}$ and $N_{\mathrm{background}}$ are the expected numbers of signal and background events (both atmospheric neutrinos and muons) with reconstructed event energy $E$ above the selected energy proxy $E_{\mathrm{event}}$ at declination $\delta$, derived from simulations.
