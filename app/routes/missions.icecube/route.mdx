...
- `observation_livetime` – duration of the analysis (seconds). The analysis duration is `1000` seconds for these analyses.
...
- `neutrino_flux_sensitivity_range`: the IceCube sensitivity assuming an E<sup>-2</sup> neutrino spectrum (E<sup>2</sup> dN/dE) found within the points in the 90% region of GW map localization. This includes two values:
  - `flux_sensitivity` – Time integrated flux sensitivity range (min, max) [GeV cm<sup>-2</sup>]. 
  - `sensitive_energy_range` – energy sensitivity range (lower, upper) [GeV].
