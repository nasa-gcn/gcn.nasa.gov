...
err_combined = np.sqrt(ra_err_mean * cos(np.deg2rad(dec)) * dec_err_mean)
```
...
example [JSON message](/schema/stable/gcn/notices/icecube/single_neutrino_alerts.example.json) files
