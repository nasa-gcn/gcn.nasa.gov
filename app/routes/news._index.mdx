...
## 2024
    ...
    <CollectionItem
      className="maxw-none"
      variantComponent={
        <CollectionCalendarDate datetime={'July 29, 2024'} />
      }
    >
      <CollectionHeading headingLevel="h3">
        <Anchor>Circulars are Now Available via Kafka, and New Schema Release v4.1.0</Anchor>
      </CollectionHeading>
      <CollectionDescription>
        ...
        - **Schema v4.1.0** includes the following updates:
          - Inclusion of [Circulars schema](https://gcn.nasa.gov/docs/schema/v4.1.0/gcn/circulars.schema.json)
          - Inclusion of the `id` property (Event core schema) in [Einstein Probe Notices](https://gcn.nasa.gov/docs/schema/v4.1.0/gcn/notices/einstein_probe/wxt/alert.schema.json)
          - Modification in the [Pointing core schema](https://gcn.nasa.gov/docs/schema/v4.1.0/gcn/notices/core/Pointing.schema.json) from `ra, dec` to `ra_pointing, dec_pointing` to remove ambiguity with source position
      </CollectionDescription>
    </CollectionItem>
