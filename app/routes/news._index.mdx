...
## 2024
    ...
    <CollectionItem
      className="maxw-none"
      variantComponent={
        <CollectionCalendarDate datetime={'May 20, 2024'} />
      }
    >
      ...
      <CollectionDescription>
        ...
        #### Einstein Probe Notices
          The Einstein Probe new GCN notice type is now available in JSON format over [Kafka](/quickstart), but are not yet available via email.  They are not available via GCN Classic.
          - Summary: EP is a mission of the [Chinese Academy of Sciences (CAS)](https://ep.bao.ac.cn/ep/), in collaboration with the [European Space Agency (ESA)](https://www.cosmos.esa.int/web/einstein-probe/mission) and the [Max Planck Institute for Extraterrestrial Physics (MPE)](https://www.mpe.mpg.de/7867826/EinsteinProbe), dedicated to time-domain high-energy astrophysics. Its primary goals are to discover high-energy transients and monitor variable objects.
          - Occurance Rate: The EP notices occur at a rate of ~100 times per year.
          - Time Delay: The post-event processing and communication of alert is distributed to GCN within ~60 seconds of detection.
          - Notice Type: The [new WXT notice type (`gcn.notices.einstein_probe.wxt.alert`)](/missions/einstein_probe) provides trigger information including localization.
        ...
        #### Astro Flavored Markdown
         The GCN Circular web form now provides an enhanced experience a custom form of Markdown, a lightweight markup language.  It allows embedding of data via tables, enriched text formatting, enhancing the already implemented automatic cross-referencing to other Circulars and common astronomy archives. [Documentation on Astro Flavored Markdown](/docs/circulars/markdown) is available.

        #### Notices Schema v4.0.0
        Explore GCN Schema via the [Schema Browser](/docs/schema/v4.0.0/gcn/notices).  Changes to the GCN Schema include:
          ...
          - Note that each JSON notice references the version of the schema it is adhering to, and `gcn.notices.icecube.lvk_nu_track_search` and `gcn.notices.swift.bat.guano` will be updated at a later date with advanced warning announced to the GCN user community.
