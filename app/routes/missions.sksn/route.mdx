...
- `detection_interval`: the time interval between the first and last neutrino events.

- `pipeline`:  
  The analysis pipeline used to generate this alert; either `"snwatch"` or `"wit"`. Both the Supernova watch (SNwatch) and Wide-band Intelligent Trigger (WIT) are online analysis system. The energy thresholds are 2.5 MeV for SNwatch and 2.5 MeV for WIT, respectively.
...
- `luminosity_distance` and `luminosity_distance_error`:  
  The estimated distance to the source and its uncertainty, assuming a SN1987A-like neutrino flux.
