...
import { NoticeFormatsAd } from './_index/NoticeFormatsAd'
...
| VOEvent Transport Protocol      | VOEvent XML                              | GCN Classic | N/A          | Retired on April 6, 2026               |
...
GCN Classic continues to maintain distribution of all legacy notice types via manual subscriptions to socket connections and email. However, we strongly encourage new sites to utilize the subscription methods of the new GCN, and for users to transition to the new system.
...
VOEvent is an XML-based text format generalized for astronomical transients. It was developed by the International Virtual Observatory Alliance ([IVOA](https://ivoa.net)). VOEvent format Notices are available via [Kafka and email](docs/notices/consuming).
