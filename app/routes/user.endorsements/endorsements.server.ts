...
import { clearUserToken } from '../_auth/user.server'
 ...
 */
export async function updateEndorsementRequestStatus(
  user: User,
  status: EndorsementState,
  requestorSub: string
) {
  if (!userIsSubmitter(user))
    throw new Response(
  ...
  let requestorMessage = `You are receiving this email because the status of your peer endorsement requested from ${user.email} has been updated to ${status}.`

  if (status === 'approved') {
    promiseArray.push(
      addUserToGroup(requestorSub),
    ...
    promiseArray.push(
      sendEmail({
        ...
        body: `${user.email} has reported the endorsement request from ${requestorEmail}.`,
      })
    )
 ...
 */
export async function deleteEndorsementRequest(
  user: User,
  endorserSub: string
) {
  const db = await tables()
 ...
 */
export async function getSubmitterUsers(user: User) {
