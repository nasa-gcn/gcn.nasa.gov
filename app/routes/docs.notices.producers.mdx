...
<ProcessList>
   ...
   <ProcessListItem>
      ...
      # When you have an event, produce to the topic
      # JSON data converted to byte string format
      # Loop to run produce when alert is generated
      data = json.dumps({
...
<Alert type="info" heading="Produce upon an event" headingLevel="h4">
The producer should always remain connected via `producer = Producer(...)`, with the JSON generation and `producer.produce` commands kicked off by new alerts from your pipeline. If you do not connect to Kafka with your credentials at least monthly, they will expire.  Implementing your alert script for constant connectivity enables that credentials do not expire and provides the lowest latency.
