...
<Alert type="info" heading="Producer Loop" headingLevel="h4">
The producer should always remain connected via `producer = Producer(...)`, with the produce loop kicked off by new alerts from your pipeline. This connectivity enables that credentials do not expire due to disuse and provides the lowest latency in distribution.
