...
Flare notices are issued only for known extragalactic sources. A source is considered to be in a flare state if its observed flux is >10x the flux reported in the 4FGL-DR4 catalog (i.e., a flux ratio ≥ 10), the detection has a Test Statistic (TS) >30, and the angular separation between the detected photons and the nearest 4FGL-DR4 source is <25 arcmin.  These are generated once per day with analysis on the prior 24 hour and 6 hour intervals.
