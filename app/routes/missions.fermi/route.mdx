...
| ------------------------------- | --------------------------------------------------- | -------------- |
| `gcn.notices.fermi.gbm.trigger` | Initial notice -- trigger info                      | ~5 seconds     |
|                                 | Flight updates with localization and classification | ~10 seconds    |
|                                 | Ground updates with finer localization              | 20–300 seconds |
|                                 | Final location update                               | 15 minutes     |

</div>

</WithFeature>

<WithFeature LAT_JSON>

**JSON-Serialized Fermi/LAT Flare Notices in GCN Kafka:**

Fermi/LAT distributes JSON-serialized notices for gamma-ray flares detected by the LAT Flare Advocate pipeline on the GCN Kafka topic `gcn.notices.fermi.lat.flare`.

Flare notices are issued only for known extragalactic sources. A source is considered to be in a flare state if its observed flux is at least 10x the flux reported in the 4FGL-DR4 catalog (i.e., a flux ratio $\geq$ 10), the detection has a Test Statistic (TS) \>30, and the angular separation between the detected photons and the nearest 4FGL-DR4 source is \<25 arcmin. These are generated once per day with analysis on the prior 24 hour and 6 hour intervals.

For schemas and examples see [/schema/stable/gcn/notices/fermi/lat](/schema/stable/gcn/notices/fermi/lat) or use the [Schema Browser](/docs/schema/stable/gcn/notices/fermi/lat/).

Detailed descriptions and examples of Fermi/LAT Flare Notices are available in the [GCN Schema GitHub project](https://github.com/nasa-gcn/gcn-schema/tree/main/gcn/notices/fermi/lat).

<div className="overflow-table">

| Topic                         | Contents                         | Latency |
| ----------------------------- | -------------------------------- | ------- |
| `gcn.notices.fermi.lat.flare` | Localization, flux, photon index | \~1 day |
