...
## 2023
    ...
    >
      ...
      <CollectionDescription>
        The Swift-BAT/GUANO, IceCube, and GCN teams are pleased to announce the availability of new GCN notice types via the new GCN in JSON format.  JSON format notices can be streamed via [Kafka](/quickstart). [Self-service email subscription](/user/email) will be available soon.  These new notice types are *not* available via GCN Classic.

        [**Swift-BAT/GUANO:**](/missions/swift#json-serialized-gcn-notice-types-in-gcn-kafka) gcn.notices.swift.bat.guano

        - *Summary:* The GUANO system is a family of gamma-ray transient search pipelines that supplement the onboard triggering capabilities of Swift/BAT, significantly enhancing the sensitivity and sky area covered by the instrument.
        - *Occurence Rate:* The Swift/BAT-GUANO notices occur at a rate of 50 times per year.
        - *Time Delay:* The post-event processing and communication of alert is distributed to GCN within 10 minutes - 5 hours of detection, depending on data downlink method and space-to-ground communications resource availability.
        ...
        - *Summary:* IceCube is a is a cubic-kilometer Cherenkov particle detector deployed in the Antarctic ice beneath the Amundsen-Scott South Pole Station. IceCube detects neutrinos by observing the light produced by relativistic charged particles produced by neutrino interactions in or near the instrumented volume of ice.
        - *Occurrence Rate:* The LVK Nu Track Search notices are triggered by LVK alerts (missions/lvk) and occur at the same rate of these alerts, searching for neutrino signals from both significant and low significance LVK alerts.
    ...
    >
      ...
      <CollectionDescription>
        This September, we've added some new features to GCN:

        - **Help Desk Ticketing with Zendesk.** Now, when you contact us through our [help desk](/contact), we create a support ticket using [Zendesk](https://www.zendesk.com). The ticketing system will help us to resolve user questions more quickly and thorougly.
      </CollectionDescription>
    </CollectionItem>
    <CollectionItem
      className="maxw-none"
