...
We suggest including a `subtype` property for situations where a single schema is used to produce multiple types of notices. This property should be customizable to fit the specific mission and should be designed as an enumerated list. For example, in space missions, the enum might include values such as ground and flight for distinct notice types.

```json
"subtype": {
"enum": ["type_1", "type_2", "type_3"],
"description": "Defines the subtype of notice. It is recommended when a single schema is used to produce multiple types of notices."
}
```
