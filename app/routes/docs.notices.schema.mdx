...
We recommend designing the new Notices schema with a minimal number of Kafka topics, preferably one schema per instrument; however, exceptions may be made when necessary. For cases where a single schema produces multiple types of notices, please include a `notice_type` field defined as an enumeration and design the enum list according to your requirements. For example, in space missions, the enum might include values such as ground and flight for distinct notice types.
