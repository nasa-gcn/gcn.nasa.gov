...
We suggest including a `subtype` property for situations where a single schema is used to produce multiple variations of this notice. This property should be customizable to fit the specific mission and should be designed as an enumerated list. For example, in space missions, the enum might include values such as ground and flight for distinct notice types.
