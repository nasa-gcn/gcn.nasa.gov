...
We suggest to include a `notice_type` defined as an enumeration, for the cases where a single schema is designed to produce multiple types of notices, please include a `notice_type` field defined as an enumeration and design the enum list according to your requirements, and an example of each usage if parameters differ. For example, in space missions, the enum might include values such as ground and flight for distinct notice types.
