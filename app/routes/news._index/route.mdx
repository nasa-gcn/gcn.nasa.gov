...
## 2025
    ...
    <CollectionItem
      ...
      variantComponent={
        <CollectionCalendarDate datetime='May 8, 2025' />
      }
    >
      ...
      <CollectionDescription>
        ...
        - [Schema v4.3.2](/news#schema-v432)
        ...
        The [Circulars archive](/circulars) now allows users to search for keywords by Subject, Body, and Submitter. Select the "Advanced Search" dropdown to view and copy example syntax. See [documentation](/docs/circulars/archive#advanced-search) for more details, examples, and compound query logic.

        ### Mission Notices Table
        The GCN [Missions Page](/missions) now contains a table listing the status of all current, recently archived, and onboarding Notices producers.  You can find what formats and distribution mechanisms are available for all Notice types, and links to prefilled email signup and the Kafka code sample generation.

        ### Notices Sample Code
        Python [Sample code](/docs/sample) serves as a starting point for consumers of GCN Notices for functions including consuming Kafka messages, parsing plain text, VOEvent XML, and JSON format notices, interacting with HEALPix maps.  We have also included producer sample code for encoding embedded data (e.g. HEALPix maps).  The GCN Team welcomes contributions of additional sample code that might be of use to our community.

        ### Apache Spark Kafka client
        A Python [Apache Spark Kafka client setup example](/docs/client#pyspark) is available for Spark users. [Apache Spark](https://spark.apache.org) is a multi-language engine for executing data engineering, data science, and machine learning on single-node machines or clusters.

        ### Schema v4.3.2
        GCN Schema v4.3.2 is available with recent changes including the generic high-energy neutrino and fast radio burst schema ([Release Notes](https://github.com/nasa-gcn/gcn-schema/releases/tag/v4.3.2)).
