...
>
    ...
    The GCN Team is pleased to announce new and updated Notice types.
      ...
      The Fermi Large Area Telescope (LAT) Team and the GCN Team are pleased to announce a new GCN JSON Notice type for [known extragalactic flaring sources](docs/schema/stable/gcn/notices/fermi/lat/flare.schema.json). A source is considered to be in a flare state if its observed flux is $\geq$10x the flux reported in the 4FGL-DR4 catalog, the detection has a Test Statistic (TS) \>30, and the angular separation between the detected photons and the nearest 4FGL-DR4 source is \<25 arcmin.  These are generated once per day with analysis on the prior 24 hour and 6 hour interval.
