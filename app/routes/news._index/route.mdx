...
<CollectionItem
    ...
    variantComponent={
      <CollectionCalendarDate datetime='August 6, 2026' />
    }
>

    <CollectionHeading headingLevel="h3">
      <Anchor>LVK Notices Exclusively Available in JSON over Kafka, Updated CHIME Notices, and Schema v7.2.3</Anchor>
    </CollectionHeading>
    <CollectionDescription>

      The GCN Team announces updates to LVK and CHIME Notices and a schema release.
        - [LVK Notices Exclusively Available in JSON over Kafka](/news#lvk-notices-exclusively-available-in-json-over-kafka)
        - [Updated CHIME Notices](/news#updated-chime-notices)
        - [Schema v7.2.3](/news#schema-v723)

      #### LVK Notices Exclusively Available in JSON over Kafka
      As of July 8, 2026, the LIGO-Virgo-KAGRA (LVK) collaboration has ceased sending gravitational wave alerts in the VOEvent format to GCN Classic. The LVK VOEvent alerts were converted to all other GCN Classic formats and distribution methods (text via email and binary via socket connection). As a result, this change also disables all legacy LVK formats (text, binary, VOEvent) distributed over Kafka and email.

      GCN now distributes LVK Notices exclusively as JSON over Kafka.  The [LVK Public Alerts User Guide](https://emfollow.docs.ligo.org/userguide/) has been updated to provide information on the alert system that will be deployed in the next observation run (IR1) that is [expected to start in late 2026](https://observing.docs.ligo.org/plan/).

      To receive GCN Notices on LVK during IR1, please [subscribe to JSON-format Notices](/docs/notices/consuming) on the Kafka topic `igwn.gwalert`.  At this time, these Notices are only available via Kafka, not email.

      #### Updated CHIME Notices

      CHIME is now distributing retractions of their FRB Notices on the topic `gcn.notices.chime.frb`, and has updated the [example](docs/schema/stable/gcn/notices/chime/frb.schema.json) added in Schema v7.2.1.

      #### Schema v7.2.3
      GCN Schema v7.2.3 is available with recent changes including updated Notice examples ([Release Notes](https://github.com/nasa-gcn/gcn-schema/releases/tag/v7.2.3)).
...
>
    ...
    <CollectionHeading headingLevel="h3">
      <Anchor>New MAXI Notices and Schema v7.2.0</Anchor>
    </CollectionHeading>
    <CollectionDescription>

      The GCN Team is pleased to announce new Notice Types and a schema release.
        - [MAXI X-ray Known and Unknown Transients Notices](/news#maxi-known-and-unknown-x-ray-transient-notices)
        - [Schema v7.2.0](/news#schema-v720)

      #### MAXI Known and Unknown X-ray Transient Notices
      The [MAXI](http://maxi.riken.jp/top/) Team and the GCN Team are pleased to announce the availability of MAXI Notices via the [new GCN](https://gcn.nasa.gov) in JSON format. MAXI is a wide-field X-ray telescope designed to perform an all-sky survey onboard the ISS.

      MAXI provides Notices for both known and unknown source detections by [Gas Slit Camera (GSC)](https://iss.jaxa.jp/en/kiboexp/ef/maxi/). The known source Notice is used when the detected event is associated with a known X-ray source or cataloged object. The unknown source Notice is used when the detected event is not immediately associated with a known source and is reported as a new or unidentified transient candidate. Depending on the available localization information, an unknown source Notice may report no position uncertainty, a circular uncertainty region, an elliptical uncertainty region, or rectangular error-region corners.

      Both Notice types provide basic alert metadata, event timing, sky position, flux information, and detection information.

      MAXI notices are published on the GCN Kafka topics `gcn.notices.maxi.gsc.known` and `gcn.notices.maxi.gsc.unknown`.

      See the [MAXI mission page](/missions/maxi) for a more extensive description of the information provided in these new notices.

      #### Schema v7.2.0
      GCN Schema v7.2.0 is available with recent changes including new Notice types ([Release Notes](https://github.com/nasa-gcn/gcn-schema/releases/tag/v7.2.0)).
  ...
  <CollectionItem
    className="maxw-none grid-container"
    variantComponent={
      <CollectionCalendarDate datetime='April 29, 2026' />
    }
>

    <CollectionHeading headingLevel="h3">
      <Anchor>New BOOM Notices and Schema v7.0.0</Anchor>
    </CollectionHeading>
    <CollectionDescription>
        The GCN Team is pleased to announce new Notice Types and a schema release.
        - [BOOM Cross-Correlated Optical Transients Notices](/news#boom-cross-correlated-optical-transient-notices)
        - [Schema v7.0.0](/news#schema-v700)

      #### BOOM Cross-Correlated Optical Transient Notices

      The [Burst & Outburst Observations Monitor (BOOM)/Babamul](https://www.ztf.caltech.edu/ztf-boom-babamul.html) Team and the GCN Team are pleased to announce the availability of a new notice type via the [new GCN](https://gcn.nasa.gov) in JSON format. BOOM provides real-time brokering, archival context, and coordination tools that enable rapid identification, vetting, and follow-up of astrophysical transients. BOOM is the software stack operating to enable a multi-survey alert broker and data archive (Babamul) that ingests and combines alerts from the Zwicky Transient Facility (ZTF) and the Vera C. Rubin Observatory alert stream.

      BOOM is contributing a GCN Notices stream, where cross-matches from the BOOM fast transient stream (which analyzes ZTF and LSST alert streams) are checked for (i) gravitational-wave alert candidates consistent with a binary neutron star merger or neutron star-black hole merger and (ii) gamma-ray bursts detected by Fermi, SVOM, and Swift. This enables rapid cross-identification of events across experiments and observatories, enhancing situational awareness and time-critical response for the broader transient astronomy community. Alert includes localization, photometry, cross-matches with other GCN Notices, and classification scores from machine-learning models.

      BOOM notices are published on the GCN Kafka topic `gcn.notices.boom.alert`.

      See the [BOOM mission page](/missions/boom) for a more extensive description of the information provided in these new notices.

      #### Schema v7.0.0
      GCN Schema v7.0.0 is available with recent changes including new Notice types and a modified and generalized Spectral core schema ([Release Notes](https://github.com/nasa-gcn/gcn-schema/releases/tag/v7.0.0)).
    </CollectionDescription>
  ...
  <CollectionItem
    className="maxw-none grid-container"
    variantComponent={
      <CollectionCalendarDate datetime='March 4, 2026' />
    }
>

    <CollectionHeading headingLevel="h3">
      <Anchor>Retirement of GCN Classic VOEvent Brokers</Anchor>
    </CollectionHeading>
    <CollectionDescription>
      As initially [announced in 2025](news#planned-retirement-of-gcn-classic-voevent-brokers), we are planning to retire the [GCN Classic VOEvent brokers](https://gcn.gsfc.nasa.gov/voevent.html) on **April 6, 2026**.  This impacts users who utilize the VOEvent Transfer Protocol (VTP) to receive VOEvent-format GCN Classic Notices. VOEvent-format Notices remain available via [Kafka and email](/docs/notices/consuming), and we recommend users migrate from VTP to Kafka.
    </CollectionDescription>
