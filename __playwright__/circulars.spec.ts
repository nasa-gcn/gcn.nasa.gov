...
test.describe('Circulars edit page', () => {
  ...
  test('submits expected values', async ({ page }) => {
    await page.goto(`/circulars/edit/${testCircular.circularId}`)
    await page.locator('#submitter').fill(testCircularEdits.submitter)
    await page
      .getByTestId('date-picker-external-input')
      .fill(testCircularEdits.date)
    await page.getByTestId('combo-box-input').fill(testCircularEdits.time)
    await page
      .getByPlaceholder('GRB 240614A: observations of')
      .fill(testCircularEdits.subject)
    await page.getByTestId('textarea').fill(testCircularEdits.body)
    await page.getByRole('button', { name: 'Update' }).click()
    await page.waitForURL('/circulars?index')
    await expect(
      page.getByRole('link', {
        name: 'ZTF and SEDM Observations of the Candidate Optical Afterglow AT 2024sva',
      })
    ).toBeVisible()
    await page
      .getByRole('link', {
    ...
    // Check new version
    /**
     * check for text being visible on page?
     */
  })
})
